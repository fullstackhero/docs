...
description: Liveness + readiness endpoints — database, Valkey, MinIO, tenant migrations — for Kubernetes / Docker Compose / load balancer probes.
sidebar:
  label: Health checks
  order: 7
pageType: concept
seo:
  title: 'Health checks in .NET 10 — Postgres + Redis + MinIO'
  description: 'How fullstackhero exposes /health and /health/ready endpoints with database (per-DbContext), Redis, MinIO, and tenant-migration health checks ready for…'
