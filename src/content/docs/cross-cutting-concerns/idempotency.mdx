...
1. **Before**: read the `Idempotency-Key` header. If present, build the cache key from the tenant (the caller's `tenant` claim, `"global"` when absent), the **operation** (HTTP method + route pattern) and the key itself, then probe the cache.
...
The reservation fails **open**. A Redis blip on reserve or release logs a warning and lets the request through rather than failing it; the stored response still dedupes later retries.
