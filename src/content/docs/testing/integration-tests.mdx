...
description: WebApplicationFactory in-process API tests against Testcontainers Postgres + Valkey + MinIO — one container set, all modules, real round trips.
sidebar:
...
seo:
  ...
  description: 'How fullstackhero runs integration tests against real Postgres + Redis + MinIO via Testcontainers, in-process API tests through WebApplicationFactory…'
...
- **`RedisFixture`** — `valkey/valkey:8-alpine` for the cache + SignalR backplane + Data Protection key ring.
