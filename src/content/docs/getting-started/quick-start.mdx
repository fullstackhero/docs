...
seo:
  ...
  description: 'Clone the fullstackhero repo and run the full stack locally via .NET Aspire: Postgres, Valkey, RustFS, migrations, the API, and both React apps with one command.'
...
Aspire brings up Postgres (with pgAdmin), Valkey, and RustFS in containers, runs the DbMigrator (migrations + seed) and the demo seeder, then starts the API and both React apps - all wired together with connection strings and OTLP export. Watch everything come up in the Aspire dashboard.
