...
`fsh-starter-rustfs-data`. If you renamed the solution, swap the prefix accordingly.
...
`redis-insight`, `rustfs`) all use `ContainerLifetime.Persistent` - they survive
...
```bash
docker ps -a
docker rm -f <stale-container>   # e.g. the redis / redis-insight / postgres / rustfs container
```
...
The integration suite runs the real API over Testcontainers (PostgreSQL + RustFS
