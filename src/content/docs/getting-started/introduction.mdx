...
pageType: guide
seo:
  title: 'What is fullstackhero? The free .NET 10 starter kit'
  description: 'fullstackhero is a free, MIT-licensed .NET 10 starter kit for multi-tenant SaaS. Modular monolith with Vertical Slice Architecture, ten modules, twelve…'
...
- **Cloud-ready.** `.NET Aspire` AppHost brings up Postgres, Valkey (a Redis-compatible, BSD-licensed Redis fork), and the API host locally with one command. Production deploy via Docker Compose included.
- **Two frontends.** React + Vite admin console and dashboard, both in `clients/`.
- **CLI + template.** Clone the repo directly today; one-command scaffolding via `fsh new` or `dotnet new fsh` is coming soon (both publish to NuGet shortly).
...
| FSH CLI *(coming soon)* | `dotnet tool install -g FullStackHero.CLI && fsh new MyApp` |
| `dotnet new` template *(coming soon)* | `dotnet new install FullStackHero.NET.StarterKit && dotnet new fsh -n MyApp` |
