...
- **CLI + template.** Clone the repo directly today. The `fsh` CLI is on NuGet as a `10.0.0-rc` prerelease; the matching `dotnet new` template hasn't shipped its 10.x version yet, so one-command scaffolding lands with the stable release.
...
| FSH CLI *(10.0.0-rc prerelease; scaffolding waits on the template)* | `dotnet tool install -g FullStackHero.CLI --prerelease` |
| `dotnet new` template *(10.x coming with the stable release)* | `dotnet new install FullStackHero.NET.StarterKit && dotnet new fsh -n MyApp` |
