...
- **Cloud-ready.** `.NET Aspire` AppHost brings up Postgres, Valkey (a Redis-compatible, BSD-licensed Redis fork), RustFS (S3-compatible object storage), the migrator, the API, and both React apps locally with one command. Production deploy via Docker Compose included.
...
| Integration      | `Microsoft.AspNetCore.Mvc.Testing`, Testcontainers (Postgres / Valkey / RustFS) |
