...
`dotnet run --project src/Host/FSH.Starter.AppHost` brings up Postgres, Valkey, RustFS, the API, the admin, and the dashboard - all at once, with service discovery handling URLs. You don't need to run `npm run dev` separately unless you're isolating frontend work.
</Callout>
...
- Postgres at tcp/5432, Valkey at tcp/6379, RustFS (S3) at :9000
