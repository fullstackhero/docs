...
<Callout type="warning" title="Prerelease — scaffolding isn't end-to-end yet">
`FullStackHero.CLI` is on NuGet as a **`10.0.0-rc` prerelease**:

```bash
dotnet tool install -g FullStackHero.CLI --prerelease
```

`fsh new` scaffolds from the `FullStackHero.NET.StarterKit` template package,
whose 10.x version isn't published yet — so it can't produce the current kit
today. Until the stable release, [clone the repo](/docs/getting-started/install/)
to get the kit. You can also run the CLI straight from a clone:
