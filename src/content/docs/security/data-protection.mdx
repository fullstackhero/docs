...
description: Valkey-backed Data Protection key persistence so cookies, antiforgery tokens, and IDataProtector payloads survive rolling deploys across instances.
sidebar:
  label: Data protection
  order: 6
pageType: concept
seo:
  title: 'Data Protection in .NET 10 - Redis-backed key persistence'
  description: 'How fullstackhero persists ASP.NET Core Data Protection keys to Redis via the shared multiplexer - so cookies, antiforgery tokens, and any…'
...
`AddHeroCaching` registers an `IConnectionMultiplexer` against Valkey, then `services.AddDataProtection().PersistKeysToStackExchangeRedis(...)` reuses the same multiplexer for key persistence. One Valkey connection pool, one key ring shared across every instance.
</Callout>
...
- **Token providers** in `UserManager` - email confirmation tokens, password reset tokens. These are encrypted+signed with Data Protection.
- **Webhook signing secrets** - the Webhooks module's `WebhookSecretProtector` encrypts each subscription's HMAC secret at rest (purpose string `FSH.Webhooks.SubscriptionSecret.v1`) and decrypts it only at dispatch time. See [webhook signing](/docs/security/webhook-signing/).
...
```csharp
// AddHeroCaching (simplified) - only when CachingOptions:Redis is set
...
```bash
dotnet user-secrets set "JwtOptions:SigningKey" "$(openssl rand -base64 32)" --project src/Host/FSH.Starter.Api
```
