...
- **`Shell(heading, innerHtml)`** - wraps markup in the kit's document: doctype, charset, viewport and the centred card. `heading` is encoded; `innerHtml` is **trusted** and inserted verbatim, so build it from literals plus `Encode`d values - never pass user input straight through.
