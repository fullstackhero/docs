...
description: How fullstackhero composes ten modules into a single deployable process with hard module boundaries enforced by architecture tests.
sidebar:
  label: Modular monolith
  order: 2
pageType: concept
seo:
  title: 'Modular monolith in .NET 10 - module boundaries enforced by'
  description: 'How fullstackhero structures ten modules as a modular monolith with hard one-way Contracts boundaries, arch-test-enforced isolation, ordered IModule…'
...
<FolderTree
  ...
  tree={[
    {
      name: 'BuildingBlocks/',
      note: 'Shared infrastructure - 11 libraries',
      chips: [
        'Core',
        'Persistence',
        'Web',
    ...
    {
      name: 'Modules/',
      note: 'Ten bounded contexts',
      children: [
        {
          name: 'Identity/',
          children: [
            { name: 'Modules.Identity/', note: 'Runtime - endpoints, handlers, domain' },
            {
              name: 'Modules.Identity.Contracts/',
              note: 'Public surface - commands, queries, DTOs, events',
              highlight: true,
            },
          ],
        },
        { name: 'Multitenancy/', badge: '+ Contracts' },
        { name: 'Auditing/', badge: '+ Contracts' },
        { name: 'Files/', badge: '+ Contracts' },
        { name: 'Chat/', badge: '+ Contracts' },
        { name: 'Notifications/', badge: '+ Contracts' },
        { name: 'Webhooks/', badge: '+ Contracts' },
        { name: 'Billing/', badge: '+ Contracts' },
        { name: 'Catalog/', badge: '+ Contracts' },
        { name: 'Tickets/', badge: '+ Contracts' },
      ],
    },
    {
      name: 'Host/',
      note: 'Composition root + orchestrator',
      children: [
        { name: 'FSH.Starter.Api/', note: 'The API process' },
        { name: 'FSH.Starter.AppHost/', note: '.NET Aspire orchestrator' },
        { name: 'FSH.Starter.DbMigrator/', note: 'Migrations + demo seeder CLI' },
        { name: 'FSH.Starter.Migrations.PostgreSQL/' },
      ],
    },
    { name: 'Tools/CLI/', note: 'The fsh CLI (Spectre.Console)' },
  ]}
/>
...
The canonical pattern. A module writes an `IIntegrationEvent` to the **outbox** (`IOutboxWriter.AddAsync`, committed with the business write); the kit's outbox dispatcher publishes it asynchronously via `IEventBus`; any module that implements `IIntegrationEventHandler<TEvent>` receives a copy, with inbox-backed idempotency. The outbox tables are framework-owned (schema `framework`), so no module registers a store of its own. Examples in the kit:

- **Chat → Notifications**: `MentionedInChannelIntegrationEvent` → inbox row + SignalR push. The kit's one deliberate direct-bus publish, so the mention badge is not delayed by a dispatch cycle.
