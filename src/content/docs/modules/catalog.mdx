...
pageType: reference
seo:
  title: 'Product Catalog in .NET 10 - brands, categories'
  description: 'How the Catalog module ships brands, hierarchical categories, products with multi-image support, price + stock domain events, soft delete, search, and…'
...
- **Files-module integration** - `ProductFileAccessPolicy` registers the `Product` owner type with the Files module: attach requires an authenticated caller (the durable gate is the product-update permission when the image lands on the product), read is public (product images ship with `Visibility=Public`), delete is uploader-only. Orphaned uploads that never attach are reaped by the Files orphan-purge job.
...
Ten integration test files in `src/Tests/Integration.Tests/Tests/Catalog/` exercise every endpoint against Testcontainers Postgres + RustFS:
